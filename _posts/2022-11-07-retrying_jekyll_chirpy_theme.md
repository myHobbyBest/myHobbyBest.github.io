...
- 사이트 clone 해오는 local 디렉토리 위치
 ...
 파일관리자를 열고 shift 키를 누른상태로 디렉토리위에서 오른쪽 마우스를 클릭하면 메뉴바가 뜨는데 이때 터미널에서 열기를 누르지 말고 __더 많은 옵션표시__ 를 선택하면 나머지 메뉴가 열린다. 여기서 다양한 터미널 종류를 중에서 적당한 것을 선택하면 터미널이 내가 원하는 디렉토리 위치에서 열린다.
