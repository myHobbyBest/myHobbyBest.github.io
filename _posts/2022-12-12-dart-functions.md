...
다른 자료에서 펌:  Lexical scope 란? runtime에 동적으로 Scope를 정하지 않고 Compile 시점에 변수나 함수가 선언된 위치를 기준으로 Scope를 정한다.
...
- Closure는 외부 함수 호출이 종료되었음에도 외부함수의 변수 또는 인자에 접근 할 수 있는 함수.
